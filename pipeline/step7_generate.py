...
입력: pipeline/output/step6_rerank.json  (없으면 step5_retrieve.json fallback)
출력: pipeline/output/step7_generate.json
...
_INPUT_CANDIDATES = [
    Path("pipeline/output/step6_rerank.json"),
    Path("pipeline/output/step5_retrieve.json"),
]
...
def main() -> None:
    input_path = next((p for p in _INPUT_CANDIDATES if p.exists()), None)
    if input_path is None:
        print("[ERROR] 입력 파일 없음. step5_retrieve.py (또는 step6_rerank.py)를 먼저 실행하세요")
        raise SystemExit(1)
