...
def build_chain(
    # ── 날짜 (chunk JSON 경로, sparse/hybrid 전용) ────────────
    chunk_date: str = None,
    # None = 오늘 날짜 자동 사용 / 예: "2026/06/19"
    # dense only일 때는 사용되지 않음
    ...
    # "sparse" : BM25 키워드 검색 (정확한 용어에 강함)
    # "hybrid" : dense + sparse 혼합 (대부분의 경우 권장)

    # ── Dense 파라미터 ────────────────────────────────────────
    search_type: str = "similarity",
    # "similarity" : 코사인 유사도 순 — 기본값
    # "mmr"        : Maximal Marginal Relevance (중복 청크 방지)
    ...
    # True  = 강제 활성화 / False = 강제 비활성화

    top_n: int = 3,
    # 리랭킹 후 LLM에 전달할 최종 문서 수 (권장: 3~5)
    ...
    # 0.0 = 결정적 / 1.0 = 창의적. RAG 권장: 0.1~0.5
